...
elements.executeButton.addEventListener("click", () => {
    ...
    if (loopConfig && loopConfig.loops.length) {
        ...
        const delayValue = Number((_b = (_a = elements.loopDelayInput) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : 0);
        const delay = Number.isFinite(delayValue) && delayValue > 0 ? delayValue : 0;
        vscode.postMessage({
            command: "executeLoop",
            text: JSON.stringify(Object.assign(Object.assign({}, loopConfig), { delay })),
        });
    }
    else {
        showGroupingMessage("");
...
elements.addUrlParam.addEventListener("click", () => createParamRow("urlParamsEditor"));
elements.addFormData.addEventListener("click", () => createParamRow("formDataEditor"));
(_a = elements.loopDelayInput) === null || _a === void 0 ? void 0 : _a.addEventListener("input", () => {
...
elements.toggleLoopButton.addEventListener("change", () => {
    ...
    if (loopPanelExpanded && !loopState.loops.length) {
        showGroupingMessage("Loops enabled. Add a loop below to get started.", "info");
    }
...
elements.toggleAdvancedButton.addEventListener("change", () => {
    var _a;
    advancedExpanded = Boolean((_a = elements.toggleAdvancedButton) === null || _a === void 0 ? void 0 : _a.checked);
    updateAdvancedVisibility();
    renderLoopEditor();
    sendReconstructCurlCommand();
});
(_b = elements.addLoopButton) === null || _b === void 0 ? void 0 : _b.addEventListener("click", () => {
    addLoop();
    showGroupingMessage("");
});
(_c = elements.addAdvancedFlag) === null || _c === void 0 ? void 0 : _c.addEventListener("click", () => {
    addAdvancedFlagRow();
    sendReconstructCurlCommand();
});
(_d = elements.advancedOptionsContainer) === null || _d === void 0 ? void 0 : _d.addEventListener("input", () => {
    sendReconstructCurlCommand();
});
[
...
[elements.independentList, elements.syncedList].forEach((zone) => {
    ...
    zone.addEventListener("drop", (event) => {
        ...
        event.preventDefault();
        const loopId = draggedLoopId || ((_a = event.dataTransfer) === null || _a === void 0 ? void 0 : _a.getData("text/plain"));
        zone.classList.remove("drag-over");
        if (!loopId) {
            return;
        }
...
function collectRouteParts() {
    ...
    const schemeRaw = (_b = (_a = elements.routeSchemeInput) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : "";
    const subdomainRaw = (_d = (_c = elements.routeSubdomainInput) === null || _c === void 0 ? void 0 : _c.value) !== null && _d !== void 0 ? _d : "";
    const domainRaw = (_f = (_e = elements.routeDomainInput) === null || _e === void 0 ? void 0 : _e.value) !== null && _f !== void 0 ? _f : "";
    const portRaw = (_h = (_g = elements.routePortInput) === null || _g === void 0 ? void 0 : _g.value) !== null && _h !== void 0 ? _h : "";
    const pathRaw = (_k = (_j = elements.routePathInput) === null || _j === void 0 ? void 0 : _j.value) !== null && _k !== void 0 ? _k : "";
    return {
...
function applyRouteParts(route) {
    var _a, _b, _c, _d, _e;
    const scheme = (_a = route === null || route === void 0 ? void 0 : route.scheme) !== null && _a !== void 0 ? _a : '';
    const subdomain = (_b = route === null || route === void 0 ? void 0 : route.subdomain) !== null && _b !== void 0 ? _b : '';
    const domain = (_c = route === null || route === void 0 ? void 0 : route.domain) !== null && _c !== void 0 ? _c : '';
    const port = (_d = route === null || route === void 0 ? void 0 : route.port) !== null && _d !== void 0 ? _d : '';
    const path = (_e = route === null || route === void 0 ? void 0 : route.path) !== null && _e !== void 0 ? _e : '';
...
function parseRouteFromUrl(url) {
    ...
    try {
        ...
        const route = Object.assign({}, empty);
...
function normalizeRouteTargetKey(value) {
    const trimmed = (value !== null && value !== void 0 ? value : '').trim().toLowerCase();
    return ROUTE_LOOP_KEYS.includes(trimmed) ? trimmed : '';
}
function inferStatusBadgeType(statusText, statusLabel) {
    const normalizedLabel = (statusLabel !== null && statusLabel !== void 0 ? statusLabel : '').toLowerCase();
    ...
    const normalizedStatus = (statusText !== null && statusText !== void 0 ? statusText : '').toLowerCase();
...
function createParamRow(containerId, key = "", value = "") {
    ...
    convertButton.addEventListener("click", () => {
        ...
        if (!targetKey) {
            showGroupingMessage("Provide a key before converting to a loop.", "error");
            return;
        }
        addLoop({
            targetType,
...
function renderParsedData(parsedData) {
    ...
    if (elements.toggleAdvancedButton) {
        elements.toggleAdvancedButton.checked =
            advancedExpanded;
    }
    updateAdvancedVisibility();
    sendReconstructCurlCommand();
...
function updateExecutionOutput(message) {
    var _a;
    elements.executionOutput.textContent = (_a = message.text) !== null && _a !== void 0 ? _a : "";
    updateSaveButtonState();
    const badges = [];
    ...
    const forceErrorBadge = labelIsError && (numericStatus === undefined || Number.isNaN(numericStatus) || numericStatus === 0);
    if (forceErrorBadge) {
...
function updateSaveButtonState() {
    const hasContent = (elements.executionOutput.textContent || "").trim().length > 0;
...
function renderLoopEditor() {
    ...
    loopState.loops.forEach((loop) => {
        ...
        const updateFlagState = () => {
            ...
            if (targetKeyInput) {
                targetKeyInput.placeholder = resolveLoopPlaceholder(loop.targetType, loop.targetFlag);
            }
            persistLoopState();
            renderGrouping();
            updateCurlPreview();
        ...
        targetKeyInput.addEventListener("blur", () => {
            ...
            if (!normalized) {
                const fallbackKey = normalizeRouteTargetKey((_a = loop.targetKey) !== null && _a !== void 0 ? _a : "") || "path";
                targetKeyInput.value = fallbackKey;
                routeKeySelect.value = fallbackKey;
                loop.targetKey = fallbackKey;
                showGroupingMessage('Route loops support "Protocol", "Subdomain", "Domain", "Port", or "Base Path".', "error");
            }
            else {
                loop.targetKey = normalized;
                targetKeyInput.value = normalized;
        ...
        valuesTextarea.value =
            (_a = loop.rawText) !== null && _a !== void 0 ? _a : loop.values.map(valueToLine).join("\n");
...
function addLoop(initialLoop) {
    const normalizedType = normalizeLoopTargetType(initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.targetType);
    const loop = {
        id: (initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.id) || generateLoopId(),
        name: (initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.name) || "",
        targetType: normalizedType,
        targetFlag: normalizeLoopFlag(normalizedType, initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.targetFlag),
        targetKey: (initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.targetKey) || "",
        rawText: (initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.rawText) || "",
        values: Array.isArray(initialLoop === null || initialLoop === void 0 ? void 0 : initialLoop.values) ? initialLoop.values : [],
    };
...
function removeLoop(loopId) {
    ...
    loopState.syncedLoopIds = loopState.syncedLoopIds.filter((id) => id !== loopId);
    if (!loopState.loops.length) {
...
function renderGrouping() {
    ...
    const independentLoops = loopState.loops.filter((loop) => !loopState.syncedLoopIds.includes(loop.id));
...
function handleDragStart(event) {
    ...
    (_a = event.dataTransfer) === null || _a === void 0 ? void 0 : _a.setData("text/plain", draggedLoopId || "");
    ...
    if (targetZone === "synced") {
        ...
        if (syncedLoops.length > 0) {
            ...
            if (referenceLength !== loop.values.length) {
                showGroupingMessage("Synchronized loops must have identical lengths.", "error");
                return;
            }
        }
        ...
        loopState.syncedLoopIds = loopState.syncedLoopIds.filter((id) => id !== loop.id);
        showGroupingMessage("");
    }
    renderGrouping();
    persistLoopState();
...
function ensureSyncedConsistency() {
    ...
    if (mismatch) {
        loopState.syncedLoopIds = [];
        showGroupingMessage("Reset synchronized loops because lengths no longer match.", "error");
    }
}
function pruneSyncedLoopIds() {
    loopState.syncedLoopIds = loopState.syncedLoopIds.filter((id) => loopState.loops.some((loop) => loop.id === id));
}
function showGroupingMessage(text, type = "info") {
    elements.loopGroupingMessage.textContent = text || "";
    elements.loopGroupingMessage.classList.toggle("error", type === "error" && !!text);
}
...
function updateAdvancedVisibility() {
    ...
    if (advancedExpanded &&
        ((_a = elements.advancedOptionsContainer) === null || _a === void 0 ? void 0 : _a.childElementCount) === 0) {
        addAdvancedFlagRow();
    }
    sendReconstructCurlCommand();
}
...
function validateLoopConfig(config) {
    ...
    for (const loop of config.loops) {
        ...
        if (loop.targetType === "route") {
            const normalizedRouteKey = normalizeRouteTargetKey((_a = loop.targetKey) !== null && _a !== void 0 ? _a : "");
    ...
    if (config.syncedLoopIds && config.syncedLoopIds.length > 0) {
        ...
        if (syncedLoops.length > 0) {
            ...
            const mismatch = syncedLoops.some((loop) => loop.values.length !== reference);
            if (mismatch) {
...
function estimateTotalRuns(config) {
    ...
    if (syncedIds.length) {
        ...
        total *= ((_a = first === null || first === void 0 ? void 0 : first.values) === null || _a === void 0 ? void 0 : _a.length) || 0;
    }
    const independentLoops = config.loops.filter((loop) => !syncedIds.includes(loop.id));
    for (const loop of independentLoops) {
        total *= ((_b = loop.values) === null || _b === void 0 ? void 0 : _b.length) || 0;
    }
    return total;
}
...
function updateLoopProgress(message) {
    var _a, _b;
    const total = (_a = message.total) !== null && _a !== void 0 ? _a : loopState.expectedTotalRuns;
    const current = (_b = message.current) !== null && _b !== void 0 ? _b : 0;
    const statusText = message.status !== undefined ? ` • Last status: ${message.status}` : "";
...
function handleLoopComplete(message) {
    ...
    showGroupingMessage("");
    if (typeof (message === null || message === void 0 ? void 0 : message.total) === "number") {
        const success = (_a = message.successCount) !== null && _a !== void 0 ? _a : 0;
        const failure = (_b = message.failureCount) !== null && _b !== void 0 ? _b : 0;
        const summaries = [
...
function addAdvancedFlagRow(flag = "", value = "") {
    ...
    convertButton.addEventListener("click", () => {
        ...
        if (!sanitizedFlag) {
            showGroupingMessage("Provide a flag before converting to a loop.", "error");
            return;
        }
...
function renderAdvancedFlags(flagStrings) {
    const container = elements.advancedOptionsContainer;
    if (!container) {
        return;
    }
    ...
    flags.forEach((entry) => {
        const text = String(entry !== null && entry !== void 0 ? entry : "").trim();
        if (!text) {
            addAdvancedFlagRow();
            return;
        }
...
function collectAdvancedFlags() {
    ...
    elements.advancedOptionsContainer
        ...
        .forEach((row) => {
        const flagInput = row.querySelector(".advanced-flag-input");
        const valueInput = row.querySelector(".advanced-value-input");
...
function sanitizeFlagNameInput(value) {
    if (typeof value !== "string") {
        return "";
    }
    return value
...
function sanitizeFlagValueInput(value) {
    if (typeof value !== "string") {
        return "";
    }
...
function getLoopTargetOptions(currentType) {
    return LOOP_TARGET_OPTIONS.filter((option) => {
        if (option.requiresAdvanced &&
            !advancedExpanded &&
...
function resolveLoopPlaceholder(type, flag) {
    if (type === "form") {
...
function normalizeLoopTargetType(type) {
    if (type === "form" || type === "header" || type === "route") {
        return type;
    }
    return "query";
}
function normalizeLoopFlag(targetType, flag) {
    ...
    const match = HEADER_FLAG_OPTIONS.find((option) => option.value.toLowerCase() === normalized.toLowerCase());
...
function headerFlagRequiresName(flag) {
    ...
    const match = HEADER_FLAG_OPTIONS.find((option) => option.value.toLowerCase() === flag.trim().toLowerCase());
    return match ? match.requiresName : true;
}
function isSupportedHeaderLoopFlag(flag) {
    const normalized = flag.trim().toLowerCase();
    return HEADER_FLAG_OPTIONS.some((option) => option.value.toLowerCase() === normalized);
}
function extractHeaderParts(text, requiresName) {
    const trimmed = text.trim();
    if (!trimmed.length) {
        return null;
    }
